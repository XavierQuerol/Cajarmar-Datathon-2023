...
PATH = "trained_models/"
model = Net()
model.load_state_dict(torch.load(PATH + "model_cas2_25.pth", map_location=torch.device('cpu')))

file = open("./trained_models/gradientboosting_cas2.obj", "rb")
model_gradient = pickle.load(file)

file = open("./trained_models/ordinal_encoder_cas2.obj", "rb")
ordinal_encoder = pickle.load(file)

file = open("./trained_models/one_hot_encoder_cas2.obj", "rb")
one_hot_encoder = pickle.load(file)

file = open("./trained_models/scaler_cas2.obj", "rb")
...
df_22 = pd.read_csv("../../DATASETS_FINALS/df_22_cas2.csv")
df_22 = df_22.astype({"CAMPAÑA": str, "ID_ZONA": str, "ID_ESTACION": str, "VARIEDAD": str, "MODO": str, "TIPO": str, "COLOR": str})

df_22_index = df_22["INDEX"]
df_22 = df_22.drop(columns=["INDEX"])
...
for batch_idx, (X_train, meteo_eto, pred_xgboost, y) in enumerate(dataloader):
    with torch.no_grad():
        model.eval()
        data1 = X_train
        data2 = meteo_eto
...
new_22.to_csv("../../PREDICTIONS/cas2_cnn.csv", index = False)
