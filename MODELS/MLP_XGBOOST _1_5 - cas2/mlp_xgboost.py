...
df_train = df_train.drop(columns = ["ID_FINCA"])

# ORDINAL ENCODING
encoder = ce.OrdinalEncoder(cols=["ID_ZONA", "ID_ESTACION", "ALTITUD", "VARIEDAD", "COLOR", "TIPO", "MODO"])
df_OrdEnc = encoder.fit_transform(df_train)
filehandler = open("./trained_models/ordinal_encoder_cas2.obj","wb")
...
# ONE HOT ENCODING
encoder = ce.OneHotEncoder(cols=["ID_ZONA", "ID_ESTACION", "VARIEDAD", "ALTITUD"])
df_OHEnc = encoder.fit_transform(df_train)
filehandler = open("./trained_models/one_hot_encoder_cas2.obj","wb")
pickle.dump(encoder, filehandler)
...
filehandler = open("./trained_models/scaler_cas2.obj","wb")
...
if final_train:
    ...
    filehandler = open("./trained_models/gradientboosting_cas2.obj","wb")
...
for epoch in range(epochs):
    ...
    if final_train:
        ...
        torch.save(model.state_dict(), PATH + "model_cas2.pth")
    else:
